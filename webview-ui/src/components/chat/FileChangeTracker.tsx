...
// Extended message types for file change tracking
interface FileChangeMessage {
	type: "getFileChangesSummary" | "getFileChangeHistory" | "exportFileChangeHistory"
	taskId: string
	filePath?: string
}
...
export const FileChangeTracker: React.FC<FileChangeTrackerProps> = ({ taskId }) => {
	...
	const sendFileChangeMessage = (message: FileChangeMessage) => {
		// Type-safe message sending with proper error boundary
		try {
			;(vscode.postMessage as (message: any) => void)(message)
		} catch (error) {
	...
	const loadChangesSummary = async () => {
		setIsLoading(true)
		try {
			sendFileChangeMessage({
				type: "getFileChangesSummary",
				taskId,
			})
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : "Unknown error occurred"
			console.error("Failed to load changes summary:", errorMessage)
		} finally {
			setIsLoading(false)
		}
	}
	...
	const loadFileChanges = async (filePath: string) => {
		...
		try {
			...
			const errorMessage = error instanceof Error ? error.message : "Unknown error occurred"
			console.error("Failed to load file changes:", errorMessage)
		} finally {
			setIsLoading(false)
		}
	}
	...
	const exportChangeHistory = async () => {
		try {
			...
			const errorMessage = error instanceof Error ? error.message : "Unknown error occurred"
			console.error("Failed to export change history:", errorMessage)
		}
	}
