...
type IntentOperationResult<T = void> = Result<T, string>
...
export class IntentManager {
	...
	async executeIntent(intentId: string): Promise<boolean> {
		...
		try {
			...
			if (checkpoint) {
				try {
					...
					this.onIntentExecuted.fire({
						...intent,
						status: "failed",
						error: `Execution and rollback both failed: ${errorMessage}`,
					})
				}
			}
			...
			throw new Error(`Failed to execute intent "${intent.description}": ${errorMessage}`)
		}
	}
...
Do you approve this operation?`
	...
	private async saveHistory(): Promise<void> {
		...
		try {
			...
			throw new Error(`Intent history save failed: ${errorMessage}`)
		}
	}
	...
	private async withHistoryLock<T extends ClineIntent | boolean | void>(operation: () => Promise<T>): Promise<T> {
		...
		this.historyUpdateMutex = previousMutex
			.then(async () => {
				try {
					...
					console.error("History operation failed:", errorMessage)
					throw error
				}
			})
			.then(() => {})
